...
function* newFunction() {
    function fn;
    return malloc(sizeof(fn.address) + sizeof(fn.stackSize) + sizeof(fn.stackSize));
}

int calSizeOfCallSite(int callSiteSizeInBytes, void* recordPtr) {
    recordPtr = (uint16_t*) recordPtr + 3; callSiteSizeInBytes = callSiteSizeInBytes + 6;
    int numOfLiveOuts = *(uint16_t*) recordPtr;
    recordPtr = (uint16_t*) recordPtr + 1; callSiteSizeInBytes = callSiteSizeInBytes + 2;
...
int readCallSite(void* recordPtr, callSiteInfo* callSiteInfo) {
    ...
    recordPtr = (uint64_t*)recordPtr + 1; callSiteSizeInBytes = callSiteSizeInBytes + 8;
    callSiteInfo -> codeOffset = *(uint32_t*)recordPtr;
    recordPtr = (uint32_t*)recordPtr + 1; callSiteSizeInBytes = callSiteSizeInBytes + 4;
    ...
    locationInfo* loc = (locationInfo*) ((uint16_t*)recordPtr + 1); callSiteSizeInBytes = callSiteSizeInBytes + 2;
...
int readFunction(functionInfo* funcPtr, void* recordPtr, function* fn) {
    ...
    callSiteInfo* callSitesInfo = calloc(fn -> callsiteCount, sizeof(callSiteInfo));
...
void readStackMap(uint64_t* t) {
    ...
    void* stackMapRecordPointer = (uint64_t*) stackMapPrt + numOfFuncs*3 + numOfConstants;
    ...
    for (size_t i = 0; i < numOfFuncs; i++) {
        int callSitesSizePerFuncInBytes = readFunction(functionPtr, stackMapRecordPointer, functionsInfo + i);
        functionPtr = functionPtr + 1;
        stackMapRecordPointer = (uint8_t*)stackMapRecordPointer + callSitesSizePerFuncInBytes;
    }
    dump();
}
